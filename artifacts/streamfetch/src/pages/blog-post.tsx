    ...
    if (line.startsWith("- ") || line.startsWith("* ")) {
      ...
      nodes.push(
        <ul key={`ul-${i}`} className="my-3 space-y-1.5 pl-4">
          {items.map((item, j) => (
            <li key={j} className="flex items-start gap-2 text-sm sm:text-base text-foreground/85">
    ...
    if (/^\d+\. /.test(line)) {
      ...
      nodes.push(
        <ol key={`ol-${i}`} className="my-3 space-y-1.5 pl-4 list-decimal list-inside">
          {items.map((item, j) => (
            <li key={j} className="text-sm sm:text-base text-foreground/85">{inlineFormat(item)}</li>
          ))}
        </ol>
      );
      continue;
...
export default function BlogPost() {
  ...
  return (
    <div className="w-full flex flex-col items-center">
      ...
      {/* Hero banner */}
      <div className={`w-full h-1.5 bg-gradient-to-r ${post.coverGradient}`} />

      <div className="w-full max-w-3xl mx-auto px-4 sm:px-6 pt-8 sm:pt-12 pb-16">
        ...
        >
          <div className="flex flex-wrap items-center gap-2 mb-5">
            <span className={`text-xs font-semibold px-2.5 py-1 rounded-full border ${categoryColors[post.category] ?? "bg-secondary text-foreground border-border"}`}>
          ...
          <div className="flex items-start gap-4 sm:gap-6 mb-6">
            <div className={`w-14 h-14 sm:w-16 sm:h-16 flex-shrink-0 rounded-2xl bg-gradient-to-br ${post.coverGradient} flex items-center justify-center text-2xl sm:text-3xl shadow-lg`}>
              {post.coverEmoji}
            </div>
          ...
          <p className="text-base sm:text-lg text-muted-foreground leading-relaxed border-l-4 border-primary/40 pl-4 mb-8 italic">
            {post.excerpt}
          </p>
        </motion.div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              {related.map((rp) => (
                <Link key={rp.slug} href={`/blog/${rp.slug}`}>
                  <div className="group h-full rounded-2xl border border-border bg-card hover:border-primary/40 hover:shadow-md transition-all duration-200 p-4 flex flex-col cursor-pointer">
                    <div className={`w-9 h-9 rounded-xl bg-gradient-to-br ${rp.coverGradient} flex items-center justify-center text-lg mb-3 shadow-sm`}>
                      {rp.coverEmoji}
                    </div>
                    <h4 className="text-sm font-semibold text-foreground group-hover:text-primary transition-colors leading-snug mb-2 line-clamp-2">
                      {rp.title}
                    </h4>
